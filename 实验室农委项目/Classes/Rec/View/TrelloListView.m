...
//    DLog(@"%ld",(long)self.currentIndex);
//    TrelloView *view = [self superview];
//    TrelloListCellItem *cell = [tableView cellForRowAtIndexPath:indexPath];
    TrelloListTableView *t_tableView = (TrelloListTableView *)tableView;
    
    TrelloListCellItem *t_item = [t_tableView.listItem.rowItemsArray objectAtIndex:indexPath.row];
//    DLog("%lu", (unsigned long)t_item.boardIndex);
    if ([self.delegate respondsToSelector:@selector(trelloListView:didSelectAtCellItem:)]) {
        [self.delegate trelloListView:self didSelectAtCellItem:t_item];
    }
