...
//    _tabView.backgroundColor = tColor;
...
#pragma mark - trelloListViewDelegate
-(void)trelloListView:(TrelloListView *)listView didSelectAtCellItem:(TrelloListCellItem *)item{
//    DLog(@"%@", item.content);
    if([self.delegate respondsToSelector:@selector(trelloItemDidSelect:)]){
        [self.delegate trelloItemDidSelect:item];
    }
}
